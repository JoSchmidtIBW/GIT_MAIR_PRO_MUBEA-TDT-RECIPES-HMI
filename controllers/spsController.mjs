...
//---------------------------------------Funktioniert-----------------------------------------------------------
export const getWriteRecipeToSPS_3 = catchAsync(async (req, res, next) => {
...
//------------------------------------------------*******------------------------------------------------------------
export const getWriteRecipeToSPS = catchAsync(async (req, res, next) => {
  console.log('bin getWriteRecipeToSPS');
  console.log('req.body.recipeData: ' + JSON.stringify(req.body.recipeData));
  const recipeToSend = req.body.recipeData;
  ...
  try {
    ...
    const vCsvName = process.env.PLC_GVL_VCSVNAME;
    console.log('vCsvName:', vCsvName);
    const spsResult_vCSvName = await writeData_WithSymbolNameAndValue(
      vCsvName,
      new_vCsvName_toWrite,
    );
    ...
    let ziehPositionenArr_toWrite = [];

    for (let i = 0; i < ziehPositionsArrUnique.length; i++) {
      let ziehPositionenObj = {
      ...
      ziehPositionenArr_toWrite.push(ziehPositionenObj);
    }
    ...
    // );
    console.log('ziehPositionenArr_toWrite:', ziehPositionenArr_toWrite);
