...
import RecipeSendSPSLog_de from '../../models/recipeSendSPSLog_de_Model.mjs';
import RecipeStatistic_de from '../../models/recipesStatistic_de_Model.mjs';
...
const DB = 'mongodb://127.0.0.1:27017/Mubea_PRO_MAIR_TDT';
...
// READ JSON-file, attention to the first-read-sequence-succession of the json's!
...
const recipeSendSPSLog_de = JSON.parse(
  fs.readFileSync(`${__dirname}/recipeSendSPSLog_de.json`, 'utf-8'),
);
const recipeStatistic_de = JSON.parse(
  fs.readFileSync(`${__dirname}/recipeStatistic_de.json`, 'utf-8'),
);
...
const importData = async () => {
  try {
    ...
    await RecipeSendSPSLog_de.create(recipeSendSPSLog_de, {
      validateBeforeSave: false,
    });
    await RecipeStatistic_de.create(recipeStatistic_de, {
      validateBeforeSave: false,
    });
...
const deleteData = async () => {
  try {
    ...
    await RecipeSendSPSLog_de.deleteMany();
    await RecipeStatistic_de.deleteMany();
